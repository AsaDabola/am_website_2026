 ...
 * home banner uses: the four buildings are each fully in frame and separated,
 * which a real oblique photograph of this campus does not give — in the
 * photograph the dormitory runs off the left edge behind the seminary.
 *
 * `shape` is the roofline traced over that rendering and `leader` the line
 * joining it to the name tag below. Both are in the image's own 1920x1080
 * space, so they line up against /images/tour/campus-aerial.jpg and no other;
 * a differently framed aerial means re-tracing all four.
 */
export type CampusBuilding = {
  ...
  /** The building's own view, cropped from the same rendering. */
  photo: string;
};
